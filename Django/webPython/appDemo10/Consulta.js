...
window.onload = function(){
	...
	btnConsultar.onclick = async function(){	
		...
		pbrConsulta.value = 0;
		divRpta.innerHTML = "";
		var rptaHttp = await fetch("ListarCodigosSizeImg");
		if(rptaHttp.ok){
...
async function obtenerDataImagenes(){
	...
	var rptaHttp = await fetch("ObtenerDataConImagenes", 
	{
		method: "POST",
	...
	if(rptaHttp.ok){
		...
		if(nTotal>0){
			...
			for(var i=0;i<nRegistros;i++){
				...
				listaAlumnos.push(lista[i]);
				x += size;
			}					
			grupoContador++;
			if(grupoContador==2) mostrarDataConImagenes();
...
function programarBotones(){
	btnBuscar.onclick = function(){
		var nRegistros = listaAlumnos.length;
		campos = [];
		...
		else alert("No se encontro el Codigo del Alumno");
	}
